...
<template>
  ...
  <div class="col-3 input-effect" style="width: 40%">
    <input type="text" placeholder="input id" v-model="id">
    <span class="focus-border">
      <i></i>
    </span>
  </div>
</div>
<el-button class="btn-1" type="danger" plain style="margin-top: 10px; margin-bottom: 10px;margin-left: 48%;" @click="deletefunc()">删除</el-button>
    </div>
</template>
<script>
import axios from 'axios';
  export default {
    ...
    async deletefunc() {
      try {
        // 发送POST请求到后端的特定端点
        const response = await axios.post('http://localhost:8088/deletesalary', {
          id: this.id,
        });
